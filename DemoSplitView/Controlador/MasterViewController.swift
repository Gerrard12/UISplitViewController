...
class MasterViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showDetailMovie" {
            guard let NavigationVC = segue.destination as? UINavigationController, let detailVC = NavigationVC.viewControllers.first as? DetailViewController else {
                fatalError("")
            }
            detailVC.movie = movieList[index]
        }
    }
}
